...
import lpips
from model.VDM_depth import VDM_depth, model_fn_decorator
...
def train_epoch(args, TrainImgLoader, model, model_fn, optimizer, epoch, iters, lr_scheduler):
    ...
    for batch_idx, data in enumerate(TrainImgLoader):
        ...
        if batch_idx % args.PRINT_FEQ ==0:
            ...
            print(desc)
            # logger.add_scalar('Train/learning_rate', optimizer.state_dict()['param_groups'][0]['lr'], (epoch-1)*num_batch+batch_idx)
...
def load_checkpoint(model, optimizer, load_epoch):
    ...
    print('Loading pre-trained adam state %s' % load_dir)
    optimizer.load_state_dict(torch.load(load_dir)['optimizers'])
...
if __name__ == '__main__':
    ...
    gpu0 = gpus[0]
    gpus = [x-gpu0 for x in gpus]
    if len(gpus)>1:
        model = nn.DataParallel(VDM_depth(args).cuda(), device_ids=gpus, output_device=gpus[0])
    elif len(gpus)==1:
        model = VDM_depth(args).cuda()
        
    optimizer = optim.Adam([{'params': model.parameters(), 'initial_lr': learning_rate}], lr=learning_rate, betas=(0.9, 0.999))
    
    if args.LOAD_EPOCH != 0:
        _, iters = load_checkpoint(model, optimizer, args.LOAD_EPOCH)
    else:
        # load pre-trained model
        if args.fhd_pretrain is not None:
            print('Loading pre-trained backbone checkpoint %s' % args.fhd_pretrain)
    ...
    loss_fn = multi_VGGPerceptualLoss(p_resize=args.P_RESIZE,lam_p=args.WEIGHT_PEC, lam_l=args.WEIGHT_L1, lam_c=args.WEIGHT_COLOR).cuda()
